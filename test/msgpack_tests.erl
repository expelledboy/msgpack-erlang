...
issue_jsx_5_test() ->
    ...
    Bin0 =
        <<130, 196, 4, 116, 121, 112, 101, 196, 7, 119, 111, 114, 107, 101, 114, 115, 196, 4, 100, 97, 116, 97, 145, 130, 196, 8, 119, 111, 114, 107, 101, 114, 105, 100, 196, 5,
            115, 116, 100, 46, 49, 196, 5, 115, 108, 111, 116, 115, 160>>,
...
unpack_test_() ->
    [
        {"not binary", ?_assertEqual({error, {badarg, []}}, unpack([]))},

        {"incomplete: null binary", ?_assertEqual({error, incomplete}, unpack(<<>>))},

        {"incomplete: unknown binary", ?_assertEqual({error, incomplete}, unpack(<<16#DA>>))}
    ].
...
map_test_() ->
    [
        ...
        {"pack map without {map_format,map}", fun() ->
...
error_test_() ->
    [
        ...
        {"badarg too big int",
            ?_assertEqual(
                {error, {badarg, -16#8000000000000001}},
                pack(-16#8000000000000001)
            )},
        {"badarg too big uint",
            ?_assertEqual(
                {error, {badarg, 16#10000000000000000}},
...
multirunner(What, Pack, Unpack) ->
    ...
    ?debugFmt(
        "~.3f s/~.3f s for ser/de ~p KB test data(~s x ~p).",
        [
            TotalPack / 1000 / 1000 / ?PCNT,
            TotalUnpack / 1000 / 1000 / ?PCNT,
            Size,
            What,
            ?PCNT
        ]
            ...
            [
                ...
                [
                    [
                        ...
                        ?_assertMatch(
                            <<16#DC, 1, 0, _:256/binary>>,
                            msgpack:pack(list_a(256), [{spec, new}, {pack_str, PackStr}])
                        ),
                        ?_assertMatch(
                            <<16#DD, 0, 1, 0, 0, _:65536/binary>>,
...
unpack_str_validation_test_() ->
    ...
    [
        ...
        {"validate_string true, on unpacking", [
            ...
            ?_assertEqual(
                {error, {badarg, binary_to_list(InvalidStr)}},
                msgpack:unpack(
                    binary_to_list(InvalidStr),
...
pack_nil_test_() ->
    ...
    [
        {"use_nil default false", [?_assertEqual({ok, <<"nil">>}, msgpack:unpack(UseNilDefaultResult))]},
        {"use_nil explicitly set to true", [?_assertEqual({ok, null}, msgpack:unpack(UseNilTrueResult))]}
    ].
...
unpack_nil_test_() ->
    ...
    [
        {"use_nil default false", [?_assertEqual({ok, null}, UseNilDefaultResult)]},
        {"use_nil explicitly set to true", [?_assertEqual({ok, nil}, UseNilTrueResult)]}
    ].
