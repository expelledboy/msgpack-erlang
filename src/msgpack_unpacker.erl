...
unpack_stream(
    <<16#D9, L:8/big-unsigned-integer-unit:1, V:L/binary, Rest/binary>>,
    ?OPTION{spec = new} = Opt
) ->
...
maybe_unpack_ext(_, Unpack, Type, <<Data/binary>>, <<Rest/binary>>, Orig, _) when
    is_function(Unpack, 3)
->
    case Unpack(Type, Data, Orig) of
        {ok, Term} -> {Term, Rest};
        {error, E} -> {error, E}
    end;
maybe_unpack_ext(_, Unpack, Type, <<Data/binary>>, <<Rest/binary>>, _, _) when
    is_function(Unpack, 2)
->
