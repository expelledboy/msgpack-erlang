...
-type ext_packer() :: fun(
    (tuple(), msgpack:options()) ->
        {ok, {Type :: byte(), Data :: binary()}}
        | {error, any()}
).
-type ext_unpacker() ::
    fun((byte(), binary(), msgpack:options()) -> {ok, msgpack_term()} | {error, any()})
    | fun((byte(), binary()) -> {ok, msgpack_term()} | {error, any()}).
...
-spec unpack(binary()) ->
    {ok, msgpack:object()}
    % a term deserilized, but binary remains
    | {error, not_just_binary}
    % too few binary to deserialize complete binary
    | {error, incomplete}
    | {error, {badarg, term()}}.
...
-spec unpack_stream(binary()) ->
    {msgpack:object(), binary()}
    | {error, incomplete}
    | {error, {badarg, term()}}
    | {error, any()}.
unpack_stream(Bin) -> unpack_stream(Bin, []).

-spec unpack_stream(binary(), msgpack:options()) ->
    {msgpack:object(), binary()}
    | {error, incomplete}
    | {error, {badarg, term()}}
    | {error, any()}.
...
parse_options([{pack_str, From} | T], Opt) when
    From =:= from_binary orelse From =:= from_list orelse From =:= from_tagged_list orelse From =:= none
->
    parse_options(T, Opt?OPTION{pack_str = From});
parse_options([{map_format, Type} | T], Opt0) when
    Type =:= jsx; Type =:= jiffy; Type =:= map
->
    Opt = Opt0?OPTION{
        map_format = Type,
...
parse_options([{ext, Module} | TL], Opt0) when is_atom(Module) ->
    Opt = Opt0?OPTION{
        ext_packer = fun Module:pack_ext/2,
...
enable_str_test() ->
    ...
    %% binary becomes str8 from_binary
    ?assertEqual(
        <<2#101:3, 7:5, "saitama">>,
